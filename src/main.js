...
function handleWomenpageClick(event) {
    event.preventDefault();
    const href = event.target.getAttribute("href");
    history.pushState({}, "", href)
    checkStateW()
...
function handleJewelrypageClick(event) {
    event.preventDefault();
    const href = event.target.getAttribute("href");
    history.pushState({}, "", href)
    checkStatej()
...
function renderJewelryPage() {
    ...
    fetch("https://fakestoreapi.com/products/category/jewelery")
        ...
        .then(json => renderjewelryData(json))
        .catch(err => alert(err));
    ...
    function renderjewelryData(data) {
        ...
        <h1 class="text-center md:text-5xl text-gray-800 font-bold py-1 md:py-4">JEWELS</h1>
        <div  class="grid grid-cols-2 md:grid-cols-4 gap-10 md:py-12 py-6 md:px-52 px-2.5  max-w-screen-2xl mx-auto bg-orange-50">
